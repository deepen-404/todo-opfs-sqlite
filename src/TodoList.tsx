        ...
        <VStack align="stretch" spacing={4} width="100%">
            {todos.map((todo) => (
                <HStack key={todo.id} p={2} bg="gray.100" borderRadius="md">
                    <Checkbox isChecked={todo.completed === 1} onChange={() => handleToggle(todo.id)} />
                    {editingId === todo.id ? (
                        <>
                        ...
                        <>
                            <Text flex={1} textDecoration={todo.completed === 1 ? 'line-through' : 'none'}>
                                {todo.text}
                            </Text>
                            <Button onClick={() => handleEdit(todo)}>Edit</Button>
                            <Button onClick={() => handleDelete(todo.id)} colorScheme="red">
                                Delete
                            </Button>
                        </>
                    )}
